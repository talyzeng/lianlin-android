...
public class xgsjhActivity extends Activity {
			...
			public void onClick(View arg0) {
				...
				if(ed_pwd1.getText().toString().equals(ed_pwd2.getText().toString())){
					handle_xgmm(ed_pwd1.getText().toString(),ed_oldpwd.getText().toString());
						 ...
						 new AlertDialog.Builder(xgsjhActivity.this).setTitle("提示").setMessage("两次输入密码不一致！")
			            	.setNegativeButton("确定",null).show(); 
					}
				}else{
				 new AlertDialog.Builder(xgsjhActivity.this).setTitle("提示").setMessage("请输入完整资料！")
	            	.setNegativeButton("确定",null).show(); 
			}*/
				if(!ed_oldpwd.getText().toString().equals("")){
					handle_xgsjh(ed_oldpwd.getText().toString());
				}else{
					new AlertDialog.Builder(xgsjhActivity.this).setTitle("提示").setMessage("请输入新手机号！")
	            	.setNegativeButton("确定",null).show(); 
				}
			}
		});
    ...
    public void handle_xgsjh(final String newsjh) {
		  pg=Chuli.c_pg(xgsjhActivity.this, "正在修改...");
        pg.show();
	      	...
	      	public void run() {
	             ...
	             try{
	            		...
	            		List <NameValuePair> params=new ArrayList<NameValuePair>();
		 				params.add(new BasicNameValuePair("sms_confirmation_answer",newsjh)); 
		...
		// String za=Chuli.HttpPatch(Chuli.yuming+BaseActivity.now_xgmmurl+"?current_password="+oldpwd+"&password="+nowpwd+"&password_confirmation="+nowpwd,BaseActivity.now_userid);
		  ...
		  //Log.v("测试返回：",za);
		  		...
		  		String za=Chuli.HttpPatch(Chuli.yuming+BaseActivity.now_user_endpoint+"/phone?sms_confirmation_answer="+newsjh, BaseActivity.now_userid);  
	        	...
	        	//cwjHandler.post(mUpdateResults_success);
		   		...
		   		//     Log.v("修改用户密码返回：",a);
		              if(za.equals("err")){
	   		        	...
	   		        	cwjHandler.post(mUpdateResults_success);
	   		         BaseActivity.now_myphone=newsjh;
		              }
    ...
    public void run() {
    	pg.dismiss();
    	 new AlertDialog.Builder(xgsjhActivity.this).setTitle("提示").setMessage("修改成功！")
    ...
    final Runnable mUpdateResults_fail = new Runnable() {
        public void run() {
        	pg.dismiss();
        	 new AlertDialog.Builder(xgsjhActivity.this).setTitle("提示").setMessage("修改失败！")
            	.setNegativeButton("确定",new android.content.DialogInterface.OnClickListener() {
